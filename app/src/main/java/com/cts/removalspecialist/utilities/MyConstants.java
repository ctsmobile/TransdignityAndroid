package com.cts.removalspecialist.utilities;

public class MyConstants {
    public static final String BASE_URL = "https://firstcall.chawtechsolutions.ch/";
    public static final String XAPIKEY = "b8da15fba00ed2a4e4f20ee12c2ba32c";
    public static String emailPattern = "[a-zA-Z0-9._-]+@[a-z]+\\.+[a-z]+";
    public static final String PREF_FCM_TOKEN = "fcm_pref";
    public static final String FCM_TOKEN = "fcm_token";
    public static  boolean isFromNotification=false;
    public static  boolean isNewTask=false;
    public static final String USER_LOGIN_PREFERENCES="user_id";
    public static final String EMAIL="email";
    public static final String DEVICE="Android";
    public static final String GROUP_ID="3";
    public static boolean isMainActivityPageOpen=false;
    public static final int LOCATION_REQUEST = 1000;
    public static final int GPS_REQUEST = 1001;
    public static final String PAYMENT_BASE_URL = "http://www.astermindscorp.com/";

}
